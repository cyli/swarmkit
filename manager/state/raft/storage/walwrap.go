...
// WALDataFromDisk contains all the data returned by a WAL's ReadAll() function
// (metadata, hardwate, and entries)
type WALDataFromDisk struct {
	Metadata  []byte
...
func ReadRepairWAL(ctx context.Context, walDir string, walsnap walpb.Snapshot, factory WALFactory) (
	...
	for {
		if reader, err = factory.Open(walDir, walsnap); err != nil {
			return nil, WALDataFromDisk{}, errors.Wrap(err, "failed to open WAL")
		}
		if metadata, st, ents, err = reader.ReadAll(); err != nil {
			if closeErr := reader.Close(); closeErr != nil {
				return nil, WALDataFromDisk{}, closeErr
			}
			if _, ok := err.(encryption.ErrCannotDecrypt); ok {
				return nil, WALDataFromDisk{}, errors.Wrap(err, "failed to decrypt WAL")
			}
			// we can only repair ErrUnexpectedEOF and we never repair twice.
			if repaired || err != io.ErrUnexpectedEOF {
				return nil, WALDataFromDisk{}, errors.Wrap(err, "irreparable WAL error")
			}
			if !wal.Repair(walDir) {
				return nil, WALDataFromDisk{}, errors.Wrap(err, "WAL error cannot be repaired")
			}
