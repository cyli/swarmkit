...
func (n *Node) loadAndStart(ctx context.Context, forceNewCluster bool) error {
	...
	n.snapshotter = encryption.NewSnapshotter(snapDir, encryption.Noop, encryption.Noop)
...
func (n *Node) createWAL(nodeID string) (raft.Peer, error) {
	...
	n.wal, err = encryption.CreateWAL(n.walDir(), metadata, encryption.Noop, encryption.Noop)
	if err != nil {
...
func (n *Node) readWAL(ctx context.Context, snapshot *raftpb.Snapshot, forceNewCluster bool) (err error) {
	...
	repaired := false
	for {
		if n.wal, err = encryption.OpenWAL(n.walDir(), walsnap, encryption.Noop, encryption.Noop); err != nil {
