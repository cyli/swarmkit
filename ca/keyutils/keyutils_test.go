...
func TestIsEncryptedPEMBlock(t *testing.T) {
	...
	for _, util := range []Formatter{Default, FIPS} {
		assert.False(t, util.IsEncryptedPEMBlock(decryptedPKCS8Block))
		assert.True(t, util.IsEncryptedPEMBlock(encryptedPKCS8Block))
	}

	// Check PKCS1 keys in non FIPS mode
	assert.False(t, Default.IsEncryptedPEMBlock(decryptedPKCS1Block))
	assert.True(t, Default.IsEncryptedPEMBlock(encryptedPKCS1Block))

	// Check PKCS1 keys in FIPS mode
	assert.False(t, FIPS.IsEncryptedPEMBlock(decryptedPKCS1Block))
	assert.False(t, FIPS.IsEncryptedPEMBlock(encryptedPKCS1Block))
}
